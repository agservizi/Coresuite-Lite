...
class Admin_UserController {
    public function list($params = []) {
        ...
        if (in_array($roleFilter, ['admin', 'operator', 'customer'], true)) {
            $filters[] = "role = ?";
            $paramsSql[] = $roleFilter;
        }
        ...
        $stmt = DB::prepare("SELECT id, name, email, phone, role, status, created_at FROM users" . $whereSql . " ORDER BY created_at DESC LIMIT ? OFFSET ?");
    ...
    public function store($params = []) {
        ...
        if ($checkStmt->fetch()) {
            $error = Locale::runtime('user_exists_email');
            include __DIR__ . '/../../Views/admin/user_form.php';
            return;
        }
        ...
        Auth::flash(Locale::runtime('user_created'), 'success');
    ...
    public function update($params = []) {
        ...
        if ($checkStmt->fetch()) {
            $error = Locale::runtime('another_user_exists_email');
            $stmt = DB::prepare("SELECT * FROM users WHERE id = ?");
            $stmt->execute([$id]);
            $user = $stmt->fetch();
            include __DIR__ . '/../../Views/admin/user_form.php';
        ...
        Auth::flash(Locale::runtime('user_updated'), 'success');
        header('Location: /admin/users');
        exit;
    }
    ...
    public function delete($params = []) {
        ...
        if ($currentUser && $currentUser['id'] == $id) {
            Auth::flash(Locale::runtime('cannot_delete_self'), 'danger');
            header('Location: /admin/users');
            exit;
        }
        ...
        Auth::flash(Locale::runtime('user_deleted'), 'success');
        header('Location: /admin/users');
        exit;
    }
}
