...
class DocumentController {
    ...
    public function list($params = []) {
        ...
        $applyTypeFilter = function ($column, $type) use (&$filters, &$paramsSql) {
            if ($type === 'pdf') {
                $filters[] = $column . " LIKE ?";
                $paramsSql[] = 'application/pdf%';
            } elseif ($type === 'image') {
                $filters[] = $column . " LIKE ?";
                $paramsSql[] = 'image/%';
            } elseif ($type === 'office') {
                $filters[] = "(" . $column . " LIKE ? OR " . $column . " LIKE ?)";
                $paramsSql[] = 'application/msword%';
                $paramsSql[] = 'application/vnd.openxmlformats-officedocument%';
            }
        };

        if (Auth::isCustomer()) {
            $baseFrom = " FROM documents WHERE customer_id = ?";
            $paramsSql[] = $user['id'];
            if ($search !== '') {
                $filters[] = "filename_original LIKE ?";
                $paramsSql[] = '%' . $search . '%';
            }
            $applyTypeFilter('mime', $typeFilter);
            ...
            $stmt = DB::prepare("SELECT *" . $baseFrom . $filterSql . " ORDER BY created_at DESC LIMIT ? OFFSET ?");
            ...
            $baseFrom = " FROM documents d JOIN users u ON d.customer_id = u.id WHERE 1=1";
            if ($search !== '') {
                $filters[] = "(d.filename_original LIKE ? OR u.name LIKE ?)";
                $like = '%' . $search . '%';
                $paramsSql[] = $like;
                $paramsSql[] = $like;
            }
            $applyTypeFilter('d.mime', $typeFilter);
            if ($customerFilter > 0) {
                $filters[] = "d.customer_id = ?";
            ...
            $stmt = DB::prepare("SELECT d.*, u.name as customer_name" . $baseFrom . $filterSql . " ORDER BY d.created_at DESC LIMIT ? OFFSET ?");
            $listParams = $paramsSql;
            $listParams[] = $perPage;
            $listParams[] = $offset;
            $stmt->execute($listParams);

            $customerStmt = DB::prepare("SELECT id, name FROM users WHERE role = 'customer' ORDER BY name");
    ...
    public function board($params = []) {
        $user = Auth::user();
        $groups = [
            'pdf' => ['label' => 'PDF', 'where' => "mime LIKE 'application/pdf%'"],
            'image' => ['label' => 'Immagini', 'where' => "mime LIKE 'image/%'"],
            'office' => ['label' => 'Office', 'where' => "(mime LIKE 'application/msword%' OR mime LIKE 'application/vnd.openxmlformats-officedocument%')"],
            'recent' => ['label' => 'Ultimi upload', 'where' => "1=1"],
        ];
        $boardDocuments = [];

        foreach ($groups as $key => $group) {
            if (Auth::isCustomer()) {
                $sql = "SELECT id, filename_original, mime, size, created_at FROM documents WHERE customer_id = ? AND " . $group['where'] . " ORDER BY created_at DESC LIMIT 10";
                $stmt = DB::prepare($sql);
                $stmt->execute([$user['id']]);
            } else {
                $sql = "SELECT d.id, d.filename_original, d.mime, d.size, d.created_at, u.name as customer_name FROM documents d JOIN users u ON d.customer_id = u.id WHERE " . str_replace('mime', 'd.mime', $group['where']) . " ORDER BY d.created_at DESC LIMIT 10";
                $stmt = DB::prepare($sql);
                $stmt->execute();
            }
            $boardDocuments[$key] = $stmt->fetchAll();
        }

        include __DIR__ . '/../Views/documents_board.php';
    }
    ...
    public function upload($params = []) {
        ...
        if (empty($customerId) || !$file || $file['error'] !== UPLOAD_ERR_OK) {
            $error = Locale::runtime('document_upload_required');
            $stmt = DB::prepare("SELECT id, name FROM users WHERE role = 'customer' ORDER BY name");
            $stmt->execute();
            $customers = $stmt->fetchAll();
            include __DIR__ . '/../Views/document_upload.php';
        ...
        if (!in_array($detectedMime, $allowedTypes) || !in_array($extension, $allowedExtensions) || $file['size'] > $maxSize) {
            $error = Locale::runtime('document_type_invalid');
            $stmt = DB::prepare("SELECT id, name FROM users WHERE role = 'customer' ORDER BY name");
            $stmt->execute();
            $customers = $stmt->fetchAll();
            include __DIR__ . '/../Views/document_upload.php';
        ...
        if (move_uploaded_file($file['tmp_name'], $uploadPath)) {
            ...
            Auth::flash(Locale::runtime('document_uploaded'), 'success');
            header('Location: /documents');
            exit;
        } else {
            $error = Locale::runtime('document_upload_error');
            $stmt = DB::prepare("SELECT id, name FROM users WHERE role = 'customer' ORDER BY name");
            $stmt->execute();
            $customers = $stmt->fetchAll();
            include __DIR__ . '/../Views/document_upload.php';
    ...
    public function delete($params = []) {
        ...
        if ($doc) {
            ...
            Auth::flash(Locale::runtime('document_deleted'), 'success');
        }
